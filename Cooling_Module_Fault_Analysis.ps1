...
if ($logFileContent.Count -gt 0) {
    ...
    $logResult | Out-File -FilePath $outputFilePath -Append -Force
	 ...
	 $faultInstance = Get-Content -Path "$outputFolder\Evidence.txt" | Where-Object { $_ -match "CRIT"}
	$summaryContent = @"
Summary :
...
-- Need to replace the [Faulted Cooling Module] with part number [Refer Thunder tool for the part number].
...
All cooling modules/FANs are in healthy state.
...
# Read the content of the SPA file
$fanInstance = Get-Content $spaFilePath -Raw

# Define the patterns for the SPA file
$patternStartFAN = "FBECLI> coolingmgmt:"
$patternEndFAN = "FBECLI> enclmgmt:"

# Use regex to extract text between the patterns
$matchedTextFAN = [regex]::Match($fanInstance, "(?s)$patternStartFAN(.*?)$patternEndFAN").Groups[1].Value

$matchedTextFAN | Out-File -FilePath $outputFilePath -Append -Force
