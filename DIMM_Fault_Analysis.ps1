...
$emcLogFilePath = ".\spa\EMC\backend\log_shared\EMCSystemLogFile.log"
...
$pattern = "(Memory\smodule)(.*)(faulted\s\(Part\sNumber\s([A-Za-z0-9]+(-[A-Za-z0-9]+)+),\sSerial\sNumber\s[A-Za-z0-9]+\))"
...
All DIMMs are in healthy state.
...
"@
	...
	# Read the content of the EMC log file and find lines containing the keyword "faulted"
	$logFileContent = Get-Content -Path $emcLogFilePath | Where-Object { $_ -match "faulted" }
...
# Use Select-String to find matches based on the pattern
...
We have analyzed the logs and found the below DIMM/DIMMs is/are showing faulted.
$matches

Action Plan :
CE needs to go onsite and perform the below :
-- Need to disable write cache and put the affected SP in service mode.
-- Need to replace the [Faulted Memore Modules] with part number [Refer summary for the part number].
